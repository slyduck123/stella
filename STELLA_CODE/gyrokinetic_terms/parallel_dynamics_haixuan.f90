...
contains
    ...
    subroutine find_departure_points
        ...
        integer :: ntime
        integer, dimension (:), allocatable :: iz_from_izext, ikx_from_izext, v_zed_from_izext, v_vpa_from_izext
                ...
                do ie = 1, neigen(iky)
                    ...
                    do iv = 1, nvpa
                        do izext = 1, nz_ext
                            ...
                            call calculate_zed_vpa_departure_idx(ntime, sub_dt, izext, iv, &
    ...
    subroutine find_tstep_count(mu, max_dvzdzed, max_dvvdzed, number_of_steps)
        ...
        max_dt = 0.2 / max(max_dvzdzed, abs(mu * max_dvvdzed)) 

        number_of_steps = ceiling(code_dt/max_dt)

    end subroutine find_tstep_count

    subroutine calculate_zed_vpa_departure_idx(no_of_steps, dt, initial_izext, initial_ivpa, &
                                v_zed_from_izext, v_vpa_from_izext, & 
                                depart_izext, depart_ivpa, departure_outside_grid)

        use zgrid, only: delzed 
        use vpamu_grids, only: vpa, dvpa ! This is is just a real number it seems for dvpa

        integer, intent(in) :: no_of_steps, initial_izext, initial_ivpa
        real, intent(in) :: dt
        ...
        timeloop: do nt = 1, no_of_steps
            ...
            do k = 1, 3 
                ! The number of iterations is somewhat arbitrarily chosen as a good compromise with the time-step size
                ! (There is almost certainly a better way of solving the implicit midpoint equation for faster convergence, 
                ! but that's not the priority right now)
                
                ! Check if projected departure point has gone outside of grid
                if (int(ivpa + d_ivpa) < 1 .or. int(ivpa + d_ivpa) > real(nvpa)) then
                    ! This requires a bit more thought hmm
                    departure_outside_grid = .true. 
                end if

                if (int(izext + d_izext) < 1 .or. int(izext_next + d_izext) >= real(nz_ext)) then
                    departure_outside_grid = .true.
                ...
                d_izext = -dt/2.0/dz * linear_interpolate(dummy_points, modulo(izext + d_izext, 1.0)) & 
                         * linear_interpolate([vpa(int(ivpa + d_ivpa)), vpa(int(ivpa + d_ivpa + 1.0))], modulo(ivpa + d_ivpa, 1.0))
    ...
    subroutine advance_parallel_dynamics 
        ...
        use array_fields, only: apar, bpar
